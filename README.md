...
The repository already contains the generated code. The following steps can be used to update the generated code should the WSDL change.

First download Metro ([1](https://eclipse-ee4j.github.io/metro-wsit/) [2](https://search.maven.org/search?q=a:metro-standalone)) and unpack into `./metro`.

We use version [3.0.2](https://jakarta.oss.sonatype.org/content/repositories/releases/org/glassfish/metro/metro-standalone/3.0.2/metro-standalone-3.0.2.zip)) as version 4.0.0 produces
```
...
This program connects to the ISDS service and list all messages, delivered in last 1440 minutes (can be set in the configuration).
